...
l_loop:		   for(int l = 0; l < 20*; l++){

#pragma HLS INLINE off
p_loop:            for(	int p = 0; p < M; p += ) {
#pragma HLS PIPELINE II = 1
j_loop:            for(int j = 0; j < ; j++) {
...
void mm( float a[M], float b[M], float c[M] )
{
	...
	float local_b[M];
	float local_c[M];
#pragma HLS ARRAY_PARTITION variable="local_a" cyclic factor= dim="1" partition
#pragma HLS ARRAY_PARTITION variable="local_b" cyclic factor= dim="1" partition
#pragma HLS ARRAY_PARTITION variable="local_c" cyclic factor= dim="1" partition
	int i,j;
	for(i = 0; i < M; i++)
	{
#pragma HLS PIPELINE
